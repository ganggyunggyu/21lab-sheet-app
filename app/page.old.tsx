...
// const SHEET_ID = '1vrN5gvtokWxPs8CNaNcvZQLWyIMBOIcteYXQbyfiZl0'; // 원본
// const SHEET_NAMES = {
//   PACKAGE: '패키지',
//   DOGMARU: '도그마루',
...
const SHEET_ID = '18HzkMhwLqlMS9UNHqMgbcSBJ4GaTcMD5qbbXQAO30c4'; // 사본
const SHEET_NAMES = {
  PACKAGE: '패키지',
  DOGMARU: '도그마루의 사본',
  DOGMARU_EXCLUDE: '도그마루 제외의 사본',
} as const;
  ...
  const handleSyncAllToDB = async () => {
    ...
    try {
      ...
      const totalDeleted =
        ...
        dogmaruSheetResult.deleted +
        dogmaruExcludeResult.deleted;
      const totalInserted =
        packageResult.inserted +
        dogmaruSheetResult.inserted +
  ...
  const handleSyncSpecific = async (type: MainTab) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          `${label} 내보내기 실패: ${errorData.error || '알 수 없는 오류'}`
        );
      }
